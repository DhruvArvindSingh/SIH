...
export default async function streetLightPost(req: Request, res: Response): Promise<void> {
    const { email, content, city, coordinates, district } = req.body;
    const fileName = `${email}-${Date.now()}`;
    let imageURL: string | null = null;
    let imageDID: string | any = null;
    try {
        imageURL = await uploadToS3(email, 'StreetLight', content, fileName);
        if (imageURL === null) {
    ...
    if (imageURL !== null && imageDID !== null) {
        ...
        if (streetLight) {
            res.status(200).json({
                success: true,
                message: "Street Light posted successfully",
            });
        }
    }
}